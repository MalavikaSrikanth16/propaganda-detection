...
class AltlexHandler:
    ...
    def findAltlexes(self, sentences):
        ...
        indices_with_causality = []
        for index, dataPoint, label in zip(indices, dataPoints, labels):
            if label:
                indices_with_causality.append(index)

        indices_with_causality = set(indices_with_causality)

        labels_sent_level = []
        for index, sentence in enumerate(sentences):
            if index in indices_with_causality:
                labels_sent_level.append(1)
            else:
                labels_sent_level.append(0)
        return labels_sent_level
